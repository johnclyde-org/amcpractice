...
def grep_png_files_from_last_10_days():
    ...
    try:
        ...
        result = subprocess.run(find_command, shell=True, check=True, stdout=subprocess.PIPE, text=True)
...
print(f'Found {len(png_urls)} PNG URLs.')

# Ask the user for the type of test
test_type = input("Are we building an (A)MC test or a (M)ATHCOUNTS test? ").strip().lower()

year = int(input("Please enter the year of the exam [2024]: ").strip())

labels = ["-"]
if test_type == "m":
    # MathCounts specific questions
    level = input("Is it (N)ationals, (S)tate, (C)hapter, or sc(H)ool: ").strip().lower()
    round_type = input("Is it (S)print, (T)arget, t(E)am, or (C)ountdown? ").strip().lower()
    full_level = {"n": "National", "s": "State", "c": "Chapter", "h": "School"}.get(level, "???")
    full_round = {"s": "Sprint", "t": "Target", "e": "Team", "c": "Countdown"}.get(round_type, "Unknown")
    test_name = f"{year} {full_level} {full_round} Round"
    labels.extend(["A", "B", "C", "D", "E"])
    for problem in range(1, 31):
        labels.append(str(problem))
elif test_type == "amc":
    # AMC specific questions
    amc_type = int(input("1) AMC-8\n2) AMC-10\n3) AMC-12\n4) AIME\n5) USAJMO\n6) USAMO\n7) IMO\nWhich one: ").strip())
    test_name = f"{year} AMC Exam"
...
problems = [{"label": label, "link": png_url} for label, png_url in zip(labels, png_urls)]
