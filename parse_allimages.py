...
test_type = (
    input("Are we building an (A)MC test or a (M)ATHCOUNTS test? ").strip().lower()
)
...
if test_type == "m":
    ...
    level = (
        input("Is it (N)ationals, (S)tate, (C)hapter, or sc(H)ool: ").strip().lower()
    )
    round_type = (
        input("Is it (S)print, (T)arget, t(E)am, or (C)ountdown? ").strip().lower()
    )
